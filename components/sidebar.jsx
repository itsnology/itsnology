...
const Sidebar = () => {
   const navigation = [
      ...
      },
      {
         ...
         icon: <IconSeo className="w-5 h-5" />,
      },
   ];
   ...
   return (
      <nav
         className={`fixed md:relative top-0 bg-sky-500 right-0 w-full lg:h-full border-l space-y-8 transition-all duration-300 ${
            isSidebarOpen
               ? "w-full h-96 md:h-full md:w-72 lg:h-full lg:w-80"
                  ...
                  {navigation.map((item, idx) => (
                     <li key={idx}>
                        <Link href={`/admin/${item.link}`}>
                           ...
                           <div
                              href={item.href}
                              className="flex items-center gap-x-2  p-2 rounded-lg text-white hover:text-gray-700 hover:bg-gray-50 active-bg-gray-100 duration-150"
                           >
                              <div>{item.icon}</div>
                              {item.name}
                           </div>{" "}
